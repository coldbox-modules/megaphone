    ...
    function run() {
        describe( "has database notifications", () => {
            it( "can get all notifications for a notifiable", () => {
                ...
                variables.megaphone.notify( userA, getInstance( "SampleNotification" ) );

                var notificationsCursor = variables.databaseNotificationService.getNotifications(
                    notifiable = userA,
                ...
                expect( notificationA.getData() ).toBe( { "foo": "bar", "notifiable": userA.getNotifiableId() } );
                ...
                expect( notificationB.getData() ).toBe( { "foo": "bar", "notifiable": userA.getNotifiableId() } );
            ...
            it( "can get all unread notifications for a notifiable", () => {
                if ( variables.isColdBox6 ) {
                    skip( "Skipping on ColdBox 6" );
                }
                ...
                var notificationsCursor = userA.getUnreadNotifications( "database" );
                expect( notificationsCursor ).toBeInstanceOf( "megaphone.models.DatabaseNotificationCursor" );
                expect( notificationsCursor.getPagination() ).toBe( {
                    "maxRows": 25,
                    "totalPages": 1,
            ...
            it( "can mark a notification as read", () => {
                ...
                var unreadNotificationsCursor = variables.databaseNotificationService.getUnreadNotifications(
                    userA,
                    "database"
                );
                ...
                var readNotificationsCursor = variables.databaseNotificationService.getReadNotifications(
                    userA,
                    "database"
                );
                ...
                var unreadNotificationsCursorAgain = variables.databaseNotificationService.getUnreadNotifications(
                    userA,
                    "database"
                );
                ...
                var readNotificationsCursorAgain = variables.databaseNotificationService.getReadNotifications(
                    userA,
                    "database"
                );
                ...
                var allNotifications = variables.databaseNotificationService.getNotifications( userA, "database" );
            ...
            it( "can mark all notifications as read", () => {
                ...
                var notificationsCursorAgain = variables.databaseNotificationService.getUnreadNotifications(
                    notifiable = userA,
                    channelName = "database",
                    initialPage = 1,
                    maxRows = 1
